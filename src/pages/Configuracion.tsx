...
type MaquinaTipo = 'impresion' | 'corte' | 'uv' | 'otro' | 'impresion_corte' | 'uv_corte';
...
interface MaquinaForm {
  ...
  costo_hora: string;
  estado: string;
  es_tercero: boolean; // true = socia externa, false = interna
}
...
const emptyMaquinaForm: MaquinaForm = {
  ...
  es_tercero: false, // por defecto es máquina propia de la empresa
};
...
function formatMaquinaTipo(tipo?: string | null) {
  switch (tipo) {
    ...
    case 'uv':
      return 'UV';
    case 'impresion_corte':
      return 'Impresión con Corte';
    case 'uv_corte':
      return 'UV con Corte';
    case 'otro':
      return 'Otro';
...
export default function Configuracion() {
  ...
  const handleMaquinaSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...
      es_tercero: maquinaForm.es_tercero,
    };
    try {
      if (editingMaquinaId) {
              ...
              <form onSubmit={handleMaquinaSubmit} style={{ padding: '20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
                ...
                <div>
                  ...
                  <select className={SEL} value={maquinaForm.tipo} onChange={e => setMaquinaForm({ ...maquinaForm, tipo: e.target.value as MaquinaTipo })}>
                    ...
                    <option value="impresion_corte">Impresión con Corte</option>
                    <option value="uv_corte">UV con Corte</option>
                ...
                <div>
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', marginTop: '8px', flexWrap: 'wrap' }}>
  <div>
    <label className={LBL}>Propiedad</label>
    <select className={SEL} value={maquinaForm.es_tercero ? 'socio' : 'empresa'} onChange={e => setMaquinaForm({ ...maquinaForm, es_tercero: e.target.value === 'socio' })}>
      <option value="empresa">Empresa (propia)</option>
      <option value="socio">Socio (externo)</option>
    </select>
  </div>
  <div>

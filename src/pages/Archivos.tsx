...
function getPedidoPdfUrl(pedido: any): string {
  if (pedido?.pdf_url) return pedido.pdf_url;
    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      {showUpload && (
        <div style={{ background: '#1E293B', border: '1px solid rgba(180,197,255,0.2)', borderRadius: '0.5rem', padding: '1.5rem' }}>
          ...
          {!STORAGE_ENABLED && (
            <div style={{ marginBottom: '14px', padding: '12px 14px', background: 'rgba(37,99,235,0.10)', border: '1px solid rgba(180,197,255,0.20)', borderRadius: '0.375rem', color: '#dae2fd', fontSize: '13px', lineHeight: 1.5 }}>
              La subida directa al storage no está activa en el navegador. Para ver o descargar comprobantes de pedidos, usa la sección{' '}
              <Link to="/pedidos-pdf" style={{ color: '#b4c5ff', fontWeight: 700, textDecoration: 'none' }}>
                Pedidos PDF
              </Link>
              . Si luego activamos storage directo, este panel también podrá manejar artes y archivos del repositorio.
            </div>
          )}
      ...
      {!STORAGE_ENABLED && (
        <div style={{ background: 'rgba(180,197,255,0.08)', border: '1px solid rgba(180,197,255,0.18)', borderRadius: '0.5rem', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span className="material-symbols-outlined" style={{ fontSize: '18px', color: '#b4c5ff' }}>info</span>
          <span style={{ fontSize: '13px', color: '#dae2fd', fontFamily: 'Inter, sans-serif' }}>
            Los comprobantes de pedidos se descargan desde{' '}
            <Link to="/pedidos-pdf" style={{ color: '#b4c5ff', fontWeight: 700, textDecoration: 'none' }}>
              Pedidos PDF
            </Link>
            . La carpeta MinIO queda para archivos del repositorio cuando esté configurada.
          </span>
        </div>
      )}
